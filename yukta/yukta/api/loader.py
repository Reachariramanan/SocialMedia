...
# Cache for compiled ecosystem data — LRU with a max of 20 entries to prevent unbounded growth
_CACHE_MAX_SIZE = 20
_compiled_cache: OrderedDict = OrderedDict()
_compiled_cache_mtime: Dict[str, float] = {}
# Pre-built O(1) index dicts per ecosystem path: {entity_type: {id: record}}
...
def _load_compiled(ecosystem_path: str) -> Dict[str, Any]:
    ...
    global _compiled_cache, _compiled_cache_mtime, _compiled_index

    path_key = str(Path(ecosystem_path).absolute())
    compiled_file = Path(ecosystem_path) / "build" / "ecosystem.yaml"

    if not compiled_file.exists():
        _compiled_cache.pop(path_key, None)
        _compiled_cache_mtime.pop(path_key, None)
        _compiled_index.pop(path_key, None)
        return {}

    current_mtime = compiled_file.stat().st_mtime

    if path_key in _compiled_cache and _compiled_cache_mtime.get(path_key) == current_mtime:
    ...
    try:
        ...
        raise EcosystemError(
            f"Failed to parse compiled ecosystem file '{compiled_file}': {e}"
        ) from e
    except IOError as e:
        raise EcosystemError(
            f"Failed to read compiled ecosystem file '{compiled_file}': {e}"
        ) from e

    # LRU eviction: move to end (most-recently-used) or insert, then evict oldest
    _compiled_cache[path_key] = data
    _compiled_cache.move_to_end(path_key)
    _compiled_cache_mtime[path_key] = current_mtime
    ...
    if len(_compiled_cache) > _CACHE_MAX_SIZE:
        ...
        _compiled_cache_mtime.pop(oldest_key, None)
...
def _lookup_compiled(ecosystem_path: str, entity_type: str, name: str) -> Optional[Dict[str, Any]]:
    ...
    path_key = str(Path(ecosystem_path).absolute())
    index = _compiled_index.get(path_key, {})
    return index.get(entity_type, {}).get(name)
...
def load_agent(name: str, path: str, use_compiled: bool = True) -> Dict[str, Any]:
    ...
    if use_compiled:
        _load_compiled(path)  # ensure cache + index are populated
        agent = _lookup_compiled(path, "agents", name)
        if agent is not None:
            agent_data = copy.deepcopy(agent)
            skill_data = resolve_agent_skills(
                agent_data.get("skills", []),
                ecosystem_path
            )
            tool_data = resolve_agent_tools(
                agent_data.get("tools", []),
                ecosystem_path
            )
